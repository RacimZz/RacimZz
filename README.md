<h1 align="center">
  <span style="color:#00bfff;">Hey, I'm <strong>Racim</strong>!</span>
</h1>

<p align="center" style="font-size: 16px;">
  <span style="color:#f1c40f;">Versatile developer</span> • 
  <span style="color:#9b59b6;">Passionate about building tech projects</span> • 
  <span style="color:#2ecc71;">From VR to AI through cybersecurity</span>
</p>
<h3 align="center" style="color:#e67e22;"><i> <a href="https://racim-zenati.vercel.app"><b style="color:#3498db;">Click here to view my portfolio</b></a> </i><br></h3>

---
<h3 align="center" style="color:#e67e22;"> Featured projects </h3>

<table align="center" width="100%">
  <tr>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/Portfolio"><b style="color:#3498db;">Portfolio</b></a><br>
      <i>My personal website showcasing my work and skills.</i><br>
      <b style="color:#e34c26;">TypeScript</b> • <b style="color:#2965f1;">CSS</b>
    </td>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/Rhino-VR"><b style="color:#9b59b6;">Rhino-VR</b></a><br>
      <i>Immersive 3D experiences built on Unity and the Rhino VR plugin.</i><br>
      <b style="color:#6c7eb7;">Unity</b> • <b style="color:#178600;">C#</b>
    </td>
  </tr>
  <tr>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/FisherEM_ML"><b style="color:#27ae60;">FisherEM_ML</b></a><br>
      <i>Unsupervised classification of galaxy spectra.</i><br>
      <b style="color:#198ce7;">R</b> • <b style="color:#f39c12;">Machine Learning</b>
    </td>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/Chatbot"><b style="color:#f39c12;">Chatbot</b></a><br>
      <i>Web interface (HTML/CSS/JS) with core logic and engine.</i><br>
      <b style="color:#3572A5;">Python</b> • <b style="color:#e34c26;">HTML</b> • <b style="color:#f1e05a;">JS</b>
    </td>
  </tr>
  <tr>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/pswrd_spray"><b style="color:#16a085;">pswrd_spray</b></a><br>
      <i>Password spraying detection system using ML pipelines to identify abnormal authentication patterns.</i><br>
      <b style="color:#3572A5;">Python</b> • <b style="color:#f39c12;">Machine Learning</b>
    </td>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/Gemclus_DeepLearning"><b style="color:#c0392b;">GemClus_DeepLearning</b></a><br>
      <i>Clustering tool based on deep learning techniques for unsupervised analysis.</i><br>
      <b style="color:#3572A5;">Python</b> • <b style="color:#f39c12;">Deep Learning</b>
    </td>
  </tr>
  <tr>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/probabilistic-voter-model"><b style="color:#16a085;">Probabilistic Voter Automaton</b></a><br>
      <i>A study of a probabilistic cellular automaton known as the voter model.</i><br>
      <b style="color:#3572A5;">Python</b>
    </td>
    <td align="center" width="45%" bgcolor="#f9f9f9" style="border-radius:10px; padding:15px; border:1px solid #ddd;">
      <a href="https://github.com/RacimZz/CNF_SAT"><b style="color:#c0392b;">Tetravex-SAT</b></a><br>
      <i>An application that automatically solves Tetravex puzzles by translating the problem into logical form (SAT) and solving it with a SAT solver.</i><br>
      <b style="color:#3572A5;">C</b> • <b style="color:#f39c12;">Ocaml</b> • <b style="color:#f39c12;">Python</b>
    </td>
  </tr>
</table>

<p align="center" style="font-size: 16px;">
   <a href="https://github.com/RacimZz?tab=repositories" style="color:#00bfff;"><b>See all my projects</b></a>
</p>


<h3 align="center" style="color:#1abc9c;">Tech stack</h3>

<p align = "center"><img src="https://skillicons.dev/icons?i=python,c,cpp,sqlite,cs,go,ocaml,html,css,js,bash,linux,unity,r,git" /></p>
<p align = "center"><img src="https://skillicons.dev/icons?i=qt,latex,kotlin" /> </p>


---
